package com.jalasoft.bootcamp.applicant.infrastructure.configuration.rabbitmq;

import static com.jalasoft.bootcamp.applicant.infrastructure.common.RabbitMQConstants.APPLICANT_QUEUE;
import static com.jalasoft.bootcamp.applicant.infrastructure.common.RabbitMQConstants.APPLICATION_FANOUT_EXCHANGE;
import static com.jalasoft.bootcamp.applicant.infrastructure.common.RabbitMQConstants.APPLICATION_TOPIC_EXCHANGE;
import static com.jalasoft.bootcamp.applicant.infrastructure.common.RabbitMQConstants.BOOTCAMP_QUEUE;
import static com.jalasoft.bootcamp.applicant.infrastructure.common.RabbitMQConstants.NOTIFICATION_QUEUE;
import static com.jalasoft.bootcamp.applicant.infrastructure.common.RabbitMQConstants.SKILL_QUEUE;
...
public class RabbitMQConfiguration {
    ...
    @Bean
    Queue defineSkillQueue()
    {
        if (skillQueue == null)
        {
            skillQueue = SKILL_QUEUE.build();
        }
        return skillQueue;
    }
    ...
    Binding defineBootcampQueueTopicExchangeBinding() {
        return BindingBuilder.bind(defineBootcampQueue()).to(defineTopicExchange())
            .with(BOOTCAMP_QUEUE.getRoutingKey());
    }
    ...
    Binding defineSkillQueueTopicExchangeBinding() {
        return BindingBuilder.bind(defineSkillQueue()).to(defineTopicExchange())
            .with(SKILL_QUEUE.getRoutingKey());
    }

    @Bean
    Binding defineSkillQueueFanoutExchangeBinding() {
        return BindingBuilder.bind(defineSkillQueue()).to(defineFanoutExchange());
    }
